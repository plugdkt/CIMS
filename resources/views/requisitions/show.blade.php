    ...
    <div class="max-w-4xl">
        ...
        @if ($canEdit)
            <div class="flex items-center gap-3 mt-6">
                <form method="POST" action="{{ route('requisitions.submit', $requisition) }}" x-data="{ submitting: false }" @submit="submitting = true">
                    @csrf
                    <button type="submit" class="rounded-lg bg-accent hover:bg-accent-strong text-white text-sm font-semibold px-5 py-2.5 disabled:opacity-50 disabled:cursor-not-allowed"
                            :disabled="submitting"
                            @disabled($requisition->items->isEmpty())>
                        <span x-show="!submitting">{{ __('requisitions.submit') }}</span>
